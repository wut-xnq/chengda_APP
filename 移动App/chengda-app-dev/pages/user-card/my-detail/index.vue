		...
		<div class="userInfo-item-wrapper">
			...
			<div class="userInfo-item" @click="onChangeAvatar">
				...
				<div class="item-right">
					...
					<Icon :size="15" color="#A6ADB6" iconClassName="arrow" type="icon-jiantou"></Icon>
				</div>
			</div>
          ...
          () =>
            navigatorToUserItem('nick', myUserInfo.nick || myUserInfo.account)
        ">
				...
				<div class="item-right">
					...
					<Icon :size="15" color="#A6ADB6" iconClassName="arrow" type="icon-jiantou">
					</Icon>
				</div>
			</div>
			<div class="box-shadow"></div>
			...
			<div class="userInfo-item" @click="
          () =>
            navigatorToUserItem('ext', myUserInfo.ext || '')
        ">
				<div class="item-left">{{ t('ext') }}*</div>
				<div class="item-right">
					<div class="ext">
						{{ myUserInfo.ext || t('unknow') }}
					</div>
					<Icon :size="15" color="#A6ADB6" iconClassName="arrow" type="icon-jiantou"></Icon>
				</div>
			</div>
			...
			<div class="box-shadow"></div>
			<!-- 性别 -->
			<picker @change="onChangeGender" :value="myUserInfo.gender == 'male' ? 0 : 1"
				:range="[t('man'), t('woman')]">
				<div class="userInfo-item">
					...
					<div class="item-right">
						<view class="uni-input">
							{{
                myUserInfo.gender === 'unknown'
                  ? t('unknow')
                  : myUserInfo.gender === 'male'
                  ? t('man')
                  : t('woman')
              }}
						</view>
			...
			<div class="box-shadow"></div>
			<!-- 生日 -->
			<picker mode="date" :value="myUserInfo.birth" @change="onChangeBirthday" :start="formatDateRange('start')"
			...
			<!-- <div
        class="userInfo-item"
        @click="() => navigatorToUserItem('tel', myUserInfo.tel || '')"
      >
        <div class="item-left">{{ t('tel') }}</div>
        <div class="item-right">
          {{ myUserInfo.tel || t('unknow')
          }}<Icon
            :size="15"
            color="#A6ADB6"
            iconClassName="arrow"
		...
		<!-- 自我介绍 -->
		<div class="signature" @click="
        () => navigatorToUserItem('signature', myUserInfo.signature || '')
      ">
...
<script lang="ts" setup>
	...
	function asyncData(type:string) {
		setTimeout(() => {
			const userInfo = uni.getStorageSync('firstLoginUser');
			updateUser({
				"userId": userInfo.user_id,
				"avatar": type === 'avatar' ? myUserInfo.value.avatar : null,
				"gender": type === 'gender' ?myUserInfo.value.gender === "male"?'1':'2':null,
				"birthday":type === 'birthday' ? myUserInfo.value.birth+" 00:00:00" : null,
			})
		}, 300)
	}

	const navigatorToUserItem = (key : string, value : string) => {
		const url = `/pages/user-card/detail-item/index?key=${key}&value=${value}`
		// console.log('navigatorToUserItem', url)
		customNavigateTo({
			url: url,
		})
	}
			...
			success: function (res) {
				if (res.networkType === 'none') {
					...
					uni.$UIKitStore.userStore
						.saveMyUserInfoActive({ ...myUserInfo.value, birth })
						.then((res : any) => {
		...
		uni.getNetworkType({
			success: function (res) {
				if (res.networkType === 'none') {
					...
					// @ts-ignore
					uni.$UIKitStore.userStore
						.saveMyUserInfoActive({ ...myUserInfo.value, gender })
						.then((res : any) => {
							myUserInfo.value = res
							asyncData("gender")
						})
    ...
    success: (modalRes) => {
      ...
      uni.chooseImage({
        ...
        success: (chooseRes) => {
          ...
          // @ts-ignore
          uni.$UIKitStore.userStore.saveMyUserInfoActive({
            ...myUserInfo.value,
            type: 'image',
            filePath,
          })
            .then((res: any) => {
              myUserInfo.value = res;
              asyncData('avatar'); // 刷新头像
              uni.showToast({ title: t('AvatarUpdated'), icon: 'success' });
            })
            .catch(() => {
              uni.showToast({ title: t('FailAvatarText'), icon: 'error' });
            })
            .finally(() => {
              uni.hideLoading();
            });
	...
	const onChangeAvatar1 = () => {
		try {
			uni.chooseImage({
				...
				//没有开启权限时，提示开启权限
				// complete: handleNoPermission,
				// complete: requestCameraPermission,
			})
		} catch (error) {
			uni.showToast({
				title: t('FailAvatarText'),
