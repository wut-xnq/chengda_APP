...
import com.jiaolong.cm.admin.api.feign.RemoteUserService;
...
public class CmAppUserDetailsServiceImpl implements CmUserDetailsService {
	...
	public UserDetails loadUserByUsername(String phone) {
		...
		if (cache != null && cache.get(phone) != null) {
			return (CmUser) cache.get(phone).get();
		}

		UserDTO userDTO = new UserDTO();
		userDTO.setPhone(phone);
		R<UserInfo> result = remoteUserService.info(userDTO);
		log.info("loadUserByUsername is :" + JSON.toJSONString(result));
	 ...
	 */
	@Override
	public UserDetails loadUserByUser(CmUser cmUser) {
		return this.loadUserByUsername(cmUser.getPhone());
	}
