 ...
 * @date 2021/12/21
 */
public interface CmUserDetailsService extends UserDetailsService, Ordered {
	...
	default UserDetails getUserDetails(R<UserInfo> result) {
		...
		if (ArrayUtil.isNotEmpty(roles)) {
			// 校验角色
			if (roles[0].intValue() != BusinessConstants.ROLE_ADMIN) {
				throw new CustomerException("不允许访问！");
			}
		...
		// 构造security用户
		return new CmUser(user.getUserId(), user.getDeptId(), user.getUsername(),
	 ...
	 * @return
	 */
	default UserDetails loadUserByUser(CmUser cmUser) {
		return this.loadUserByUsername(cmUser.getUsername());
	}
