...
package com.jiaolong.cm.common.xss.core;

import cn.hutool.core.util.CharsetUtil;
import com.jiaolong.cm.common.xss.config.CmXssProperties;
import com.jiaolong.cm.common.xss.utils.XssUtil;
...
public class DefaultXssCleaner implements XssCleaner {
	...
	private final CmXssProperties properties;

	public DefaultXssCleaner(CmXssProperties properties) {
		this.properties = properties;
	}

	private static Document.OutputSettings getOutputSettings(CmXssProperties properties) {
	...
	public String clean(String bodyHtml, XssType type) {
		...
		CmXssProperties.Mode mode = properties.getMode();
		if (CmXssProperties.Mode.escape == mode) {
			// html 转义
			return HtmlUtils.htmlEscape(bodyHtml, CharsetUtil.UTF_8);
		}
		else if (CmXssProperties.Mode.validate == mode) {
			// 校验
