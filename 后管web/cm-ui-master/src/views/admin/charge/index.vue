		...
		<div class="layout-padding-auto layout-padding-view">
			<el-row class="ml10" v-show="showSearch">
				<el-form :inline="true" :model="state.queryForm" @keyup.enter="getDataList" ref="queryRef">
					...
					<el-form-item :label="$t('charge.paymentType')" prop="paymentType">
						<el-select clearable v-model="state.queryForm.payment_type">
							<el-option v-for="(item, index) in payment_type" :key="index" :label="item.label" :value="item.value"></el-option>
						</el-select>
					</el-form-item>
					...
					<el-form-item :label="$t('charge.totalPrice')" prop="totalPrice">
						<el-input-number clearable :placeholder="'￥'" style="max-width: 150px" v-model="state.queryForm.amountMin" />
						-
						<el-input-number clearable :placeholder="'￥'" style="max-width: 150px" v-model="state.queryForm.amountMax" />
					</el-form-item>
			...
			<el-row>
				<div class="mb8" style="width: 100%">
					<el-button plain :disabled="multiple" v-auth="'admin_purch_del'" @click="handleDelete(selectObjs)" class="ml10" icon="Delete" type="primary">
						{{ $t('common.delBtn') }}
					</el-button>
				</div>
			</el-row>
			...
			>
				...
				<el-table-column :label="t('charge.createTime')" min-width="100" prop="createTime" show-overflow-tooltip />
				...
				<el-table-column width="90px" :label="t('charge.paymentState')" prop="paymentState" show-overflow-tooltip>
					<template #default="scope">
						{{ getLabelByArr(scope.row.paymentState, payment_state) }}
					</template>
				</el-table-column>
				<el-table-column label="支付宝交易号" prop="tradeFlowNo" show-overflow-tooltip />
				<el-table-column :label="$t('common.action')" width="100">
					<template #default="scope">
						<el-button icon="delete" v-auth="'admin_purch_del'" @click="handleDelete([scope.row.id])" text type="primary">
							{{ $t('common.delBtn') }}
						</el-button>
					</template>
				</el-table-column>
...
import { delObj, fetchList } from '/@/api/admin/charge';
