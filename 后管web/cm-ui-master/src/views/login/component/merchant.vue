<template>
  <el-form size="large" class="login-content-form" ref="loginFormRef2" :rules="loginRules" :model="state.ruleForm"
...
const loginFormRef2 = ref(); // 定义LoginForm表单引用
...
const onSignIn = async () => {
  const valid = await loginFormRef2.value.validate().catch(() => {}); // 表单校验
  if (!valid) return false;

  loading.value = true; // 正在登录中
  try {
    await useUserInfo().loginForMerchant(state.ruleForm); // 调用登录方法
