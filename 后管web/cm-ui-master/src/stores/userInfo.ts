...
import { getUserInfo, login, loginByMobile, loginBySocial, refreshTokenApi, loginForMerchant } from '/@/api/login/index';
	...
	actions: {
		...
		async setUserInfos() {
			await getUserInfo().then((res) => {
				...
				// 	const merchantId =
				getObj(userInfo.user.userId).then((res) => {
					userInfo.merchantId = res.data.merchantId;
				});
				console.log(345, userInfo);
		 ...
		 * @returns {Promise<Object>}
		 */
		async loginForMerchant(data: any) {
			data.grant_type = 'password';
			data.scope = 'server';

			return new Promise((resolve, reject) => {
				loginForMerchant(data)
					.then((res) => {
						// 存储token 信息
						Session.set('token', res.access_token);
						Session.set('refresh_token', res.refresh_token);
